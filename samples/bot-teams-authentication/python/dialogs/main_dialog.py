...
class MainDialog(LogoutDialog):
    ...
    async def display_token_phase2(
        ...
        if step_context.result:
            # Create message with feedback buttons enabled
            message_text = f"Here is your token {step_context.result.token}"
            
            # Create activity with feedback buttons enabled
            feedback_activity = MessageFactory.text(message_text)
            feedback_activity.channel_data = {
                "feedbackLoop": {
                    "type": "default"  # Use default feedback form
                }
            }
            
            await step_context.context.send_activity(feedback_activity)
